...
/*
 ** Process Detector fhog
 */

#include <pthread.h>
#include <mutex>
// check frontal_face_detector.h:15
#define THREADS_COUNT 6
extern pthread_t   gg_threads[6];
template <
    typename pyramid_type,
    typename feature_extractor_type,
...
struct  t_threaddata
{
    ...
    (
     ...
     int             filter_rows_padding,
     int             filter_cols_padding
    ) : _mutex_dets(_mutex_dets), feats(feats), fe(fe), w(w), thresh(thresh),
    dets(dets), l(l), pyr(pyr),
    det_box_height(det_box_height), det_box_width(det_box_width),
    cell_size(cell_size), filter_rows_padding(filter_rows_padding),
...
typename fhog_filterbank
            >
void    apply_filters_to_fhog_threaded(
        t_threaddata<pyramid_type, feature_extractor_type, fhog_filterbank>    *d
        )
{
    std::mutex& _mutex_dets = *(d->_mutex_dets);

    array2d<float> my_saliency_image;
    const rectangle area = apply_filters_to_fhog(*(d->w), (*(d->feats))[d->l], my_saliency_image);
    // now search the saliency image for any detections
    std::vector<std::pair<double, rectangle>>&  dets = *(d->dets);
    for (long r = area.top(); r <= area.bottom(); ++r)
    {
        for (long c = area.left(); c <= area.right(); ++c)
        {
            // if we found a detection
            if (my_saliency_image[r][c] >= d->thresh)
            {
                rectangle rect = (*d->fe).feats_to_image(centered_rect(point(c,r),d->det_box_width,d->det_box_height),
                        d->cell_size, d->filter_rows_padding, d->filter_cols_padding);
                rect = (*d->pyr).rect_up(rect, d->l);
                {
                    std::lock_guard<std::mutex> lock(_mutex_dets);
                    dets.push_back(std::make_pair(my_saliency_image[r][c], rect));
                }
            }
        }
    }
...
{
    t_threaddata<pyramid_type, feature_extractor_type, fhog_filterbank>    *gg_threaddata[THREADS_COUNT];
    std::mutex      _mutex_dets;
    ...
    {
        int tid;
        gg_threaddata[l] = new t_threaddata<pyramid_type, feature_extractor_type, fhog_filterbank>(
                &_mutex_dets,
                &feats,
                &fe,
                &w,
        ...
        auto func = (void*(*)(void*))(apply_filters_to_fhog_threaded<pyramid_type, feature_extractor_type, fhog_filterbank>);
